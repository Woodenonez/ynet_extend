	...
	with torch.no_grad():
		...
		for trajectory, meta, scene in val_loader:
			...
			for i in range(0, len(trajectory), batch_size):
				print(f'Scene: {cnt}/{len(val_loader)}. Trajectory: {i+1}/{len(trajectory)}')
	...
	with torch.no_grad():
		...
		for trajectory, meta, scene in val_loader:
			...
			for i in range(0, len(trajectory), batch_size):
				...
				[ax.cla() for ax in axes.ravel()[0:]]
				axes[0,0].imshow(background[0,:].cpu().permute(1,2,0).numpy()[:,:,::-1])
				...
				pred_waypoint_map_sigmoid = pred_waypoint_map / temperature
				axes[0,1].imshow(pred_waypoint_map_sigmoid[0,-1,:].cpu(), cmap='gray')

				pred_waypoint_map_sigmoid = model.sigmoid(pred_waypoint_map_sigmoid) ### NOTE very important
				...
				axes[0,0].plot(goal_samples.squeeze(1).squeeze(1).cpu()[:,0], goal_samples.squeeze(1).squeeze(1).cpu()[:,1], 'yx')
				...
				for tr in future_samples:
					axes[0,0].plot(tr.squeeze(0).cpu()[:,0], tr.squeeze(0).cpu()[:,1])
				...
				axes[0,0].plot(observed[:,0], observed[:,1], 'b.')
				axes[0,0].plot(gt_goal.squeeze(0).cpu()[0,0], gt_goal.squeeze(0).cpu()[0,1], 'rp')
